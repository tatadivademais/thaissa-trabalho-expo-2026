...
const ENCODED_UTF8 = new RegExp(['%[0-7][\\dA-F]', '%(?:C[2-9A-F]|D[\\dA-F])%[89AB][\\dA-F]', '%E0%[AB][\\dA-F]%[89AB][\\dA-F]', '%E[1-9A-CEF](?:%[89AB][\\dA-F]){2}', '%ED%[89][\\dA-F]%[89AB][\\dA-F]', '%F0%[9AB][\\dA-F](?:%[89AB][\\dA-F]){2}', '%F[1-3](?:%[89AB][\\dA-F]){3}', '%F4%8[\\dA-F](?:%[89AB][\\dA-F]){2}'].join('|'), 'gi');
const decode = value => {
  if (!value.includes('%')) {
    return value;
  }
  try {
    ...
    return value.replace(ENCODED_UTF8, sequence => decodeURIComponent(sequence));
  }
};
const encode = value => {
