...
import { getStateFromRouteParams } from "./getStateFromRouteParams.js";
import * as queryString from "./queryString.js";
import { validatePathConfig } from "./validatePathConfig.js";
const serializeParamValue = value => value === null ? null : Array.isArray(value) ? value.map(String) : String(value);
const getActiveRoute = (state, configs, getRouteState) => {
...
export function getPathFromState(state, options) {
  ...
  const cachedRouteStates = new Map();
  const getRouteState = (route, config) => {
    if (route.state) {
      return route.state;
    }
    const hasScreenParams = route.params && 'screen' in route.params && typeof route.params.screen === 'string' && config?.screens?.[route.params.screen];
    const hasStateParams = route.params && 'state' in route.params && config?.screens;
    if (!route.params || !config?.screens || !hasScreenParams && !hasStateParams) {
      return undefined;
    }
  ...
  while (current) {
    ...
    while (route.name in currentOptions && hasNext) {
      ...
      if (route.params) {
        ...
        const currentParams = Object.fromEntries(Object.entries(route.params).map(([key, value]) => {
          if (value === undefined) {
            if (options) {
              const optional = ownParts.find(part => part.param === key)?.optional;
              if (optional) {
                return null;
              }
            } else {
          ...
          return [key, stringify ? stringify(value) : serializeParamValue(value)];
        ...
        if (focusedRoute === route) {
          ...
          for (const key in currentParams) {
            ...
            if (!claimedParams.has(key) && value !== undefined && value !== 'undefined') {
      ...
      if (!config.screens || routeState === undefined) {
        ...
        index = typeof routeState.index === 'number' ? routeState.index : routeState.routes.length - 1;
    ...
    if (!focusedParams && focusedRoute.params) {
      focusedParams = Object.fromEntries(Object.entries(focusedRoute.params).map(([key, value]) => [key, serializeParamValue(value)]));
    }
    if (routeState) {
      path += '/';
...
const createConfigItem = (config, parentParts) => {
  ...
  const parts = config.exact !== true ? [...(parentParts || []), ...ownParts] : ownParts.length ? ownParts : undefined;
