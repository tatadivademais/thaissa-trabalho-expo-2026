...
function BottomSheetContent(StyledBottomSheetContent) {
    return forwardRef((_a, ref) => {
        ...
        const { visible, handleClose, trapFocus, initialFocusRef, handleCloseBackdrop, finalFocusRef, snapPoints, snapToIndex, preventScroll, } = React.useContext(BottomSheetContext);
        ...
        const [windowHeight, setWindowHeight] = React.useState(() => Platform.OS === 'web'
        ...
        React.useEffect(() => {
            const handleDimensionChange = ({ window: windowDim }) => {
                const newHeight = Platform.OS === 'web' && typeof window !== 'undefined'
                    ? window.innerHeight || windowDim.height
                    : windowDim.height;
        ...
        React.useEffect(() => {
            if (visible) {
                ...
                const initialSnapHeight = (snapPoints === null || snapPoints === void 0 ? void 0 : snapPoints[snapToIndex]) || 50;
        ...
        const contentSheetAnimatePosition = React.useMemo(() => {
            ...
            const snapPointIndex = Math.min(snapToIndex, snapPoints.length - 1);
        ...
        if (_experimentalContent) {
            return (<StyledBottomSheetContent {...props} ref={mergedRef} {...dialogProps} onLayout={(event) => {
                    const { height } = event.nativeEvent.layout;
                    contentSheetHeight.current = height;
                }}>
            <BottomSheetContentProvider contentSheetHeight={contentSheetHeight} pan={pan} handleClose={handleCloseCallback} handleCloseBackdrop={handleCloseBackdrop} snapPoints={snapPoints} activeSnapIndex={activeSnapIndex} setActiveSnapIndex={setActiveSnapIndex} maxSnapPoint={maxSnapPoint} windowHeight={windowHeight}>
              {children}
        ...
        return (<Animated.View style={{
          ...
          <StyledBottomSheetContent {...props} style={[
                props.style,
                {
                    height: snapPoints && snapPoints.length > 0
...
//# sourceMappingURL=BottomSheetContent.jsx.map